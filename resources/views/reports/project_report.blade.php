                    ...
                    <div class="ol-card-body p-3 mb-10 position-relative" id="filters-container">
                                    ...
                                    <div class="top-bar d-flex align-items-center w-100 flex-wrap">
                                        ...
                                        <div class="custom-dropdown" id="export-btn1">
                                            ...
                                            <ul class="dropdown-list dropdown-export">
                                                <li class="mb-1">
                                                    <a class="dropdown-item export-btn" href="javascript:void(0)" onclick="exportFile('{{ route(get_current_user_role() . '.project-report.export-file', ['file' => 'pdf']) }}')"><i class="fi-rr-file-pdf"></i>
                                                        {{ get_phrase('PDF') }}</a>
                                                </li>
                                                <li class="mb-1">
                                                    <a class="dropdown-item export-btn" href="javascript:void(0)" onclick="exportFile('{{ route(get_current_user_role() . '.project-report.export-file', ['file' => 'csv']) }}')">
                                                ...
                                                <li>
                                                    <a class="dropdown-item export-btn" href="javascript:void(0)" onclick="exportFile('{{ route(get_current_user_role() . '.project-report.export-file', ['file' => 'print']) }}')"><i class="fi-rr-print"></i>
                                                        {{ get_phrase('Print') }}</a>
                                                </li>
                                            </ul>
                                        </div>
                                ...
                                <tbody>
                                    @php
                                        ...
                                        if (auth()->user()->role_id == 1) {
                                            ...
                                            $total_project_reports = $query->get()->count();
                                        } else {
                                    ...
                                    @foreach ($query->limit(10)->get() as $key => $report)
                                        <tr>
                                            <td>
                                                <div class="d-flex align-items-center">
                                            ...
                                            <td>
                                                {{ date('d M Y h:i A', $report->date_added) }}
                                            </td>
                                            <td>
                                                {{ App\Models\Project::where('code', $report->project_code)->first()?->title }}
                                            </td>
                                            <td>
                                                {{ currency($report->total_amount) }}
                                            </td>
                                            <td>
                                                {{ $report->payment_types }}
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
                        ...
                        <div class="d-none d-lg-block custom-datatable-row-selector">
                            <div class="page-length-select fs-12px margin--40px d-flex align-items-center position-absolute">
                                ...
                                <label for="page-length-select" class="ps-2 w-100"> {{ get_phrase('of') }} {{ $total_project_reports }}</label>
                            </div>
                        </div>
                    </div>
                </div>
...
@push('js')
    ...
    <script>
        ...
        initiate_floating_actions(
            ...
            "", // Delete route
            "{{ route(get_current_user_role() . '.project-report.export-file', ['file' => 'pdf']) }}", // Export route pdf
            "{{ route(get_current_user_role() . '.project-report.export-file', ['file' => 'csv']) }}", // Export route csv
            "{{ route(get_current_user_role() . '.project-report.export-file', ['file' => 'print']) }}", // Export route for print copy
        );
        ...
        function applyFilterToGraphs() {
            ...
            setTimeout(() => {
                $.ajax({
                    url: "{{ route(get_current_user_role().'.project_report_chart') }}",
                    method: "get",
        ...
        server_side_datatable('["id","date","project","amount","payment_types"]', "{{ route(get_current_user_role() . '.project_report') }}", '{{ $total_project_reports }}');
        ...
        function pieChart(paymentsJson) {
            let dataArr = paymentsJson.map(payment => ({
            ...
            if (donutChart) {
                ...
                var donutOptions = {
                    ...
                    colors: [
                        '#212534', '#4e97ff', '#4de78e', '#98B9EC',
                        '#98ecca', '#acec98', '#eeee84', '#efb082',
                        '#e87777', '#8b8e9a'
                    ],
                    dataLabels: {
                        enabled: false
                ...
                donutChart = new ApexCharts(document.querySelector("#donut"), donutOptions);
                donutChart.render();
            }
        }

        function barChart(paymentsJson) {
            let colors = [
                '#000', '#1b84ff', '#4de78e', '#98B9EC', '#98ecca',
                '#acec98', '#eeee84', '#efb082', '#e87777', '#8b8e9a'
            ];

            let dataArr = paymentsJson.map((payment, index) => ({
                x: payment.project,
