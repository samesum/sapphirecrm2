                ...
                <div class="ol-card">
                    <div class="ol-card-body p-3 mb-10 position-relative" id="filters-container">
                                    ...
                                    <div class="top-bar flex-wrap d-flex align-items-center w-100">
                                        ...
                                        <div class="custom-dropdown" id="export-btn1">
                                            ...
                                            <ul class="dropdown-list dropdown-export">
                                                <li class="mb-1">
                                                    <a class="dropdown-item export-btn" href="javascript:void(0)" onclick="exportFile('{{ route(get_current_user_role() . '.client-report.export-file', ['file' => 'pdf']) }}')"><i class="fi-rr-file-pdf"></i>
                                                        {{ get_phrase('PDF') }}</a>
                                                </li>
                                                <li class="mb-1">
                                                    <a class="dropdown-item export-btn" href="javascript:void(0)" onclick="exportFile('{{ route(get_current_user_role() . '.client-report.export-file', ['file' => 'csv']) }}')">
                                                ...
                                                <li>
                                                    <a class="dropdown-item export-btn" href="javascript:void(0)" onclick="exportFile('{{ route(get_current_user_role() . '.client-report.export-file', ['file' => 'print']) }}')"><i class="fi-rr-print"></i>
                                                        {{ get_phrase('Print') }}</a>
                                                </li>
                                            </ul>
                                        </div>
                        ...
                        <div class="table-responsive">
                            <table class="table server-side-datatable" id="report_list">
                                <thead>
                                ...
                                <tbody>
                                    @php
                                        ...
                                        if (auth()->user()->role_id == 1) {
                                            $query = $query->select('user_id', 'user_id', DB::raw('MAX(date_added) as date_added'), DB::raw('SUM(amount) as total_amount'), DB::raw('GROUP_CONCAT(DISTINCT payment_type SEPARATOR ", ") as payment_types'))->groupBy('user_id');
                                        } else {
                                            $query
                                                ->where(function ($q) use ($userId) {
                                                    $q->whereHas('project', function ($q) use ($userId) {
                                                        $q->where('client_id', (string) $userId)->orWhereRaw('JSON_CONTAINS(staffs, JSON_QUOTE(?))', [(string) $userId]);
                                                    });
                                                })
                                        ...
                                        $total_client_reports = $query->get()->count();
                                    @endphp

                                    @foreach ($query->limit(10)->get() as $key => $history)
                                        <tr>
                                            <td>
                                                <div class="d-flex align-items-center">
                                            ...
                                            <td>
                                                {{ date('d M Y h:i A', $history->date_added) }}
                                            </td>
                                            <td>
                                                {{ $history->user?->name }}
                                            </td>
                                            <td>
                                                {{ currency($history->total_amount) }}
                                            </td>
                                            <td>
                                                {{ $history->payment_types }}
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
                        ...
                        <div class="d-none d-lg-block custom-datatable-row-selector">
                            <div class="page-length-select fs-12px margin--40px d-flex align-items-center position-absolute">
                                ...
                                <label for="page-length-select" class="ps-2 w-100"> {{ get_phrase('of') }} {{ $total_client_reports }}</label>
                            </div>
                        </div>
                    </div>
                </div>
...
@push('js')
    ...
    <script>
        ...
        initiate_floating_actions(
            '#report_list', // table or any parent element
            "", // Delete route
            "{{ route(get_current_user_role() . '.client-report.export-file', ['file' => 'pdf']) }}", // Export route pdf
            "{{ route(get_current_user_role() . '.client-report.export-file', ['file' => 'csv']) }}", // Export route csv
            "{{ route(get_current_user_role() . '.client-report.export-file', ['file' => 'print']) }}", // Export route for print copy
        );

        server_side_datatable('["id","date", "client", "amount","payment_types"]', "{{ route(get_current_user_role() . '.client_report') }}", '{{ $total_client_reports }}');


        function applyFilterToGraphs() {
            var d = {}; // ✅ Use an object, not an array

            $('#filters-container :input').each(function() {
                var name = $(this).attr('name');
                var value = $(this).val();
                if (name) {
                    d[name] = value || null; // ✅ Add to object
                }
            });

            setTimeout(() => {
                $.ajax({
                    url: "{{ route(get_current_user_role() . '.client_report_chart') }}",
                    method: "get",
                    data: d, // ✅ Now it's a proper object
                    headers: {
        ...
        const interval = setInterval(() => {
            if (typeof pieChart === 'function' && typeof barChart === 'function') {
                pieChart(@json($payments));
                barChart(@json($payments));
                clearInterval(interval); // Stop checking once functions are available and called
            }
        }, 500);


        function barChart(paymentsJson, height = 300) {
            console.log(paymentsJson);

            let dataArr = paymentsJson.map(function(payment) {
            ...
            var barOptions = {
                ...
                title: {
                    text: '{{ get_phrase('Client Reports') }}',
                    align: 'left'
                }
            };
        ...
        function pieChart(paymentsJson, height = 300) {
            let dataArr = paymentsJson.map(function(payment) {
                return {
    ...
    <script>
        'use strict';
        @php
            $max_value = App\Models\Payment_history::max('amount');
        @endphp
