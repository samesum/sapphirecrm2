...
@push('title', get_phrase('Manage Language'))
@push('meta')@endpush
@push('css')@endpush
@section('content')
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <div class="form-group">
                                                    <form action="#" class="d-flex gap-2 align-items-bottom">
                                            ...
                                            <td class="">
                                                <a href="{{ route('admin.language.phrase.edit', ['lan_id' => $language->id]) }}" class="btn ol-btn-primary px-3 py-2 text-13px mb-1">{{ get_phrase('Edit phrase') }}</a>

                                                @if ($language->name == 'english' || $language->name == 'English')
                                                @else
                                                    <a href="javascript:;" onclick="confirmModal('{{ route('admin.language.delete', ['id' => $language->id]) }}')" class="btn ol-btn-danger px-3 py-2 mb-1 text-13px">{{ get_phrase('Delete') }}</a>
                                                @endif
                                            </td>
                                        </tr>
                                    @endforeach
                                ...
                                <div class="col-md-8 fpb-7 mb-2">
                                    ...
                                    <div class="upload-container">
                                        <div class="drop-zone">
                                            ...
                                            <input type="file" name="language_file" class="file-input" hidden accept=".json" required>
                                        </div>
...
@push('js')
    <script type="text/javascript">
        ...
        function updatePhrase(key, key_main) {
            ...
            var currentEditingLanguage = '{{isset($current_editing_language) ? $current_editing_language : ''}}';
            $.ajax({
                type: "POST",
                url: "",
                data: {
                ...
                success: function(response) {
                    ...
                    success('{{get_phrase('phrase_updated')}}');
                }
            });
        }
