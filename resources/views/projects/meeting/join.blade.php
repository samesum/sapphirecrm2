...
<head>
    <title>{{ $meeting->title }} | {{ get_phrase('Meeting') }}</title>
    <meta charset="utf-8" />
...
<body>
    ...
    @php
        ...
        $project = App\Models\Project::where('id', $meeting->project_id)->first();
        $is_host = $project->user_id == auth()->user()->id ? 1 : 0;
    @endphp
    ...
    <script>
        ...
        var lang = "en-US";
        var china = 0;
        var sdkKey = "7M6Wg3sxRP6fRudLqqskYQ"; //SDK Key or Client ID
        var sdkSecret = "z1NzSPndVwGqmquWnoJgza2i2R4GJOai"; //SDK Secret or Client Secret
