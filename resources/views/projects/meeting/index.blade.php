...
@push('title', get_phrase('Meeting').' - '.$project->title)
                ...
                <div class="ol-card-body p-3 position-relative" id="filters-container">
                                ...
                                <div class="top-bar d-flex align-items-center w-100 flex-wrap">
                                    ...
                                    <div class="custom-dropdown" id="export-btn1">
                                        ...
                                        <ul class="dropdown-list dropdown-export">
                                            <li class="mb-1">
                                                <a class="dropdown-item export-btn" href="javascript:void(0)" onclick="exportFile('{{ route(get_current_user_role() . '.meeting.export-file', ['file' => 'pdf', 'code' => request()->route()->parameter('code')]) }}')">
                                                    <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                        <path
                                            ...
                                            <li class="mb-1">
                                                <a class="dropdown-item export-btn" href="javascript:void(0)" onclick="exportFile('{{ route(get_current_user_role() . '.meeting.export-file', ['file' => 'csv', 'code' => request()->route()->parameter('code')]) }}')">
                                            ...
                                            <li>
                                                <a class="dropdown-item export-btn" href="javascript:void(0)" onclick="exportFile('{{ route(get_current_user_role() . '.meeting.export-file', ['file' => 'print', 'code' => request()->route()->parameter('code')]) }}')">
                                                    <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    ...
                                    @if (has_permission('meeting.create'))
                                        <button onclick="rightCanvas('{{ route(get_current_user_role() . '.meeting.create', ['code' => request()->route()->parameter('code')]) }}', 'Create meeting')" class="btn ol-btn-outline-secondary d-flex align-items-center cg-10px ms-auto enable-no-data-action">
                        ...
                        <table class="table server-side-datatable milestone-table" id="project_list">
                            <thead>
                                <tr class="context-menu-header">
                                    ...
                                    <th scope="col">{{ get_phrase('Agenda') }}</th>
                            ...
                            <tbody>
                                ...
                                @foreach ($query->limit(10)->get() as $key => $meeting)
                                    <tr>
                                        ...
                                        <td>{{ $meeting?->title }}</td>
                                        <td>{{ date('d M Y h:i A', strtotime($meeting->timestamp_meeting)) }}</td>
                                        <td>
                                            @if ($meeting->status == 'upcoming')
                                                <span class="badge bg-warning">{{ get_phrase('Upcoming') }}</span>
                                            @elseif($meeting->status == 'ongoing')
                                                <span class="badge bg-danger">{{ get_phrase('Ongoing') }}</span>
                                            @elseif($meeting->status == 'ended')
                                                <span class="badge bg-light text-secondary">{{ get_phrase('Ended') }}</span>
                                            @endif
                                        </td>
                                        ...
                                        <td>
                                            @php
                                                ...
                                                if ($meetingInfo) {
                                                    ...
                                                    echo '<a class="join-btn" target="_blank" href="' . $url . '">' . get_phrase('Start Meeting') . '</a>';
                                                } else {
                                                    echo get_phrase('Offline');
                                                }
                                            @endphp
                                        </td>
                                        <td>
                                            @php
                                                $editRoute = route($current_user_role . '.meeting.edit', $meeting->id);
                                                $deleteRoute = route($current_user_role . '.meeting.delete', $meeting->id);
                                                $options = 0;
                                            @endphp
                                            ...
                                            <div class="dropdown disable-right-click ol-icon-dropdown">
                                                ...
                                                <ul class="dropdown-menu contextMenuContainer">
                                                    @if (has_permission('meeting.edit'))
                                                        @php $options++; @endphp
                                                        <li>
                                                            <a class="dropdown-item" onclick="rightCanvas('{{ $editRoute }}', '{{ get_phrase('Edit meeting') }}')" href="javascript:void(0)">{{ get_phrase('Edit') }}</a>
                                                        </li>
                                                    @endif
                    ...
                    <div class="d-none d-lg-block custom-datatable-row-selector">
                        <div class="page-length-select fs-12px margin--40px d-flex align-items-center position-absolute">
                            ...
                            <label for="page-length-select" class="ps-2 w-100"> {{ get_phrase('of') }} {{ $total_meetings }}</label>
                        </div>
                    </div>
...
@push('js')
    <script>
        ...
        initiate_floating_actions(
            ...
            "{{ has_permission('meeting.delete') ? route(get_current_user_role() . '.meeting.multi-delete'):'' }}", // Delete route
            "{{ route(get_current_user_role() . '.meeting.export-file', ['file' => 'pdf', 'code' => request()->route()->parameter('code')]) }}", // Export route pdf
            "{{ route(get_current_user_role() . '.meeting.export-file', ['file' => 'csv', 'code' => request()->route()->parameter('code')]) }}", // Export route csv
            "{{ route(get_current_user_role() . '.meeting.export-file', ['file' => 'print', 'code' => request()->route()->parameter('code')]) }}", // Export route for print copy
        );

        server_side_datatable('["id","title","time","status","agenda","join","options"]', "{{ route(get_current_user_role() . '.meetings') }}", '{{ $total_meetings }}');
    </script>
@endpush
