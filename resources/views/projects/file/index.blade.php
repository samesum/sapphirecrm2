...
@push('title', get_phrase('File').' - '.$project->title)
@php
                ...
                <div class="ol-card-body p-3 position-relative" id="filters-container">
                                ...
                                <div class="top-bar d-flex align-items-center w-100 flex-wrap">
                                    ...
                                    <div class="custom-dropdown" id="export-btn1">
                                        ...
                                        <ul class="dropdown-list dropdown-export">
                                            <li class="mb-1">
                                                <a class="dropdown-item export-btn" href="javascript:void(0)" onclick="exportFile('{{ route(get_current_user_role() . '.file.export-file', ['file' => 'pdf', 'code' => request()->route()->parameter('code')]) }}')">
                                                    <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                        <path
                                            ...
                                            <li class="mb-1">
                                                <a class="dropdown-item export-btn" href="javascript:void(0)" onclick="exportFile('{{ route(get_current_user_role() . '.file.export-file', ['file' => 'csv', 'code' => request()->route()->parameter('code')]) }}')">
                                            ...
                                            <li>
                                                <a class="dropdown-item export-btn" href="javascript:void(0)" onclick="exportFile('{{ route(get_current_user_role() . '.file.export-file', ['file' => 'print', 'code' => request()->route()->parameter('code')]) }}')">
                                                    <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    ...
                                    <div class="custom-dropdown filter-dropdown btn-group" id="export-btn">
                                        ...
                                        <div class="dropdown-list custom-dropdown-280px px-14px" id="filter-section">
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <select class="px-14px form-select ol-form-control ol-select2" name="uploaded_by" id="uploaded_by">
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <select class="form-control px-14px ol-form-control ol-select2" name="size" id="size">
                                                    ...
                                                    <option value="1|5">{{ get_phrase('1MB - 5MB') }}</option>
                                                    <option value="5|10">{{ get_phrase('5MB - 10MB') }}</option>
                                                    <option value="10|1000000">{{ get_phrase('10MB <= ') }}</option>
                                                </select>
                                            </div>
                                            ...
                                            <!-- Apply Button -->
                                            <button type="button" id="filter" class="btn btn-apply px-14px w-100 mt-2" onclick="applyFilter('table-filter')">{{ get_phrase('Apply') }}</button>
                                        </div>
                                    </div>
                                    @if (has_permission('file.create'))
                                        <button onclick="rightCanvas('{{ route(get_current_user_role() . '.file.create', ['code' => request()->route()->parameter('code')]) }}', 'Create file')" class="btn ol-btn-outline-secondary d-flex align-items-center cg-10px ms-auto enable-no-data-action">
                        ...
                        <table class="table server-side-datatable file-datatable" id="project_list">
                            <thead>
                                <tr class="context-menu-header">
                                    ...
                                    @if (has_permission('file.download') || auth()->user()->role_id == 1)
                            ...
                            <tbody>
                                ...
                                @foreach ($query->limit(10)->get() as $key => $file)
                                    <tr>
                                        <td>
                                        ...
                                        <td>
                                            {{ $file?->title }}
                                            <p class="text-muted text-9px">{{ strtoupper($file->extension) }} | {{ $file->size . ' ' . get_phrase('MB') }}</p>
                                        </td>
                                        <td>{{ date('d M Y h:i A', strtotime($file->timestamp_start)) }}</td>
                                        <td>{{ App\Models\User::where('id', $file->user_id)->firstOrNew()->name }}</td>
                                        @if (has_permission('file.download') || auth()->user()->role_id == 1)
                                            <td>
                                                <a href="{{ asset($file->file) }}" download="{{ $file?->title }}.{{ $file->extension }}" class="download-btn">
                                                    <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                        <path
                                        ...
                                        <td>
                                            @php
                                                $editRoute = route($current_user_role . '.file.edit', $file->id);
                                                $deleteRoute = route($current_user_role . '.file.delete', $file->id);
                                                $options = 0;
                                            @endphp
                                            ...
                                            <div class="dropdown disable-right-click ol-icon-dropdown">
                                                ...
                                                <ul class="dropdown-menu contextMenuContainer">
                                                    ...
                                                    @if (has_permission('file.edit'))
                                                        @php $options++; @endphp
                                                        <li>
                                                            <a class="dropdown-item" onclick="rightCanvas('{{ $editRoute }}', '{{ get_phrase('Edit file') }}')" href="javascript:void(0)">{{ get_phrase('Edit') }}</a>
                                                        </li>
                                                    @endif
                    ...
                    <div class="d-none d-lg-block custom-datatable-row-selector">
                        <div class="page-length-select fs-12px margin--40px d-flex align-items-center position-absolute">
                            ...
                            <label for="page-length-select" class="ps-2 w-100"> {{ get_phrase('of') }} {{ $total_files }}</label>
                        </div>
                    </div>
...
@push('js')
    ...
    <script>
        ...
        initiate_floating_actions(
            ...
            "{{has_permission('file.delete') ? route(get_current_user_role() . '.file.multi-delete'):'' }}", // Delete route
            "{{ route(get_current_user_role() . '.file.export-file', ['file' => 'pdf', 'code' => request()->route()->parameter('code')]) }}", // Export route pdf
            "{{ route(get_current_user_role() . '.file.export-file', ['file' => 'csv', 'code' => request()->route()->parameter('code')]) }}", // Export route csv
            "{{ route(get_current_user_role() . '.file.export-file', ['file' => 'print', 'code' => request()->route()->parameter('code')]) }}", // Export route for print copy
        );
        ...
        if (role != 1 && !"{{ has_permission('file.download') }}") {
            hasRolePermission = '["id","title","date","updated_by","options"]';
        } else if ("{{ has_permission('file.download') }}") {
            hasRolePermission = '["id","title","date","updated_by","downloaded","options"]';
        } else {
            hasRolePermission = '["id","title","date","updated_by","downloaded","options"]';
        }

        server_side_datatable(hasRolePermission, "{{ route(get_current_user_role() . '.files') }}", '{{ $total_files }}');
    </script>
@endpush
