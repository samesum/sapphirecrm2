...
@push('title', get_phrase('Timesheet') . ' - ' . $project->title)
                ...
                <div class="ol-card-body p-3 position-relative" id="filters-container">
                                ...
                                <div class="top-bar d-flex align-items-center w-100 flex-wrap">
                                    ...
                                    <div class="custom-dropdown" id="export-btn1">
                                        ...
                                        <ul class="dropdown-list dropdown-export">
                                            <li class="mb-1">
                                                <a class="dropdown-item export-btn" href="javascript:void(0)" onclick="exportFile('{{ route(get_current_user_role() . '.timesheet.export-file', ['file' => 'pdf', 'code' => request()->route()->parameter('code')]) }}')">
                                                    <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                        <path
                                            ...
                                            <li class="mb-1">
                                                <a class="dropdown-item export-btn" href="javascript:void(0)" onclick="exportFile('{{ route(get_current_user_role() . '.timesheet.export-file', ['file' => 'csv', 'code' => request()->route()->parameter('code')]) }}')">
                                            ...
                                            <li>
                                                <a class="dropdown-item export-btn" href="javascript:void(0)" onclick="exportFile('{{ route(get_current_user_role() . '.timesheet.export-file', ['file' => 'print', 'code' => request()->route()->parameter('code')]) }}')">
                                                    <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    ...
                                    <div class="custom-dropdown dropdown filter-dropdown btn-group" id="export-btn">
                                        ...
                                        <!-- Dropdown Menu -->
                                        <div class="dropdown-menu px-14px" aria-labelledby="filterDropdownButton" id="filter-section">
                                    ...
                                    @if (has_permission('timesheet.create'))
                                        <button onclick="rightCanvas('{{ route(get_current_user_role() . '.timesheet.create', ['code' => request()->route()->parameter('code')]) }}', 'Timesheet')" class="btn ol-btn-outline-secondary d-flex align-items-center cg-10px ms-auto enable-no-data-action">
                    ...
                    <div class="table-responsive">
                        <table class="table server-side-datatable" id="project_list">
                            ...
                            <tbody>
                                ...
                                @foreach ($query->limit(10)->get() as $key => $time)
                                    <tr>
                                        <td>
                                        ...
                                        <td>
                                            @php
                                                $teamStaff = App\Models\User::where('id', $time->staff)->firstOrNew();
                                            @endphp

                                            <ul class="ovelaped-user-list">
                                                <li>
                                                    <img src="{{ $teamStaff->photo ? get_image($teamStaff->photo) : get_image('assets/global/images/default.jpg') }}" alt="" data-bs-toggle="tooltip" title="{{ $teamStaff->name }}">
                                                </li>
                                            </ul>
                                        </td>
                                        <td>{{ date('d M Y h:i A', strtotime($time->timestamp_start)) }}</td>
                                        <td>{{ date('d M Y h:i A', strtotime($time->timestamp_end)) }}</td>
                                        <td>
                                            @php
                                            ...
                                            @endphp
                                            {!! $hours . ' ' . removeScripts(get_phrase('Hours')) !!}
                                        </td>
                                        <td>
                                            @php
                                                $editRoute = route($current_user_role . '.timesheet.edit', $time->id);
                                                $deleteRoute = route($current_user_role . '.timesheet.delete', $time->id);
                                                $options = 0;
                                            @endphp
                                            ...
                                            <div class="dropdown disable-right-click ol-icon-dropdown">
                                                ...
                                                <ul class="dropdown-menu contextMenuContainer">
                                                    @if (has_permission('timesheet.edit'))
                                                        @php $options++; @endphp
                                                        <li>
                                                            <a class="dropdown-item" onclick="rightCanvas('{{ $editRoute }}', '{{ get_phrase('Edit project') }}')" href="javascript:void(0)">{{ get_phrase('Edit') }}</a>
                                                        </li>
                                                    @endif
                    ...
                    <div class="d-none d-lg-block custom-datatable-row-selector">
                        <div class="page-length-select fs-12px margin--40px d-flex align-items-center position-absolute">
                            ...
                            <label for="page-length-select" class="ps-2 w-100"> {{ get_phrase('of') . ' ' . $total_timesheets }}</label>


                        </div>
                    </div>
                    <div class="d-none d-md-block">
                        <div class="row justify-content-start justify-content-lg-center mt--50px total-hour-container d-none">
                            <div class="col-auto">
                                <button class="btn btn-outline-primary mb--10px text-12px">{{ get_phrase('Total Hours:') }} <span id="numberOfTotalHours"></span> {{ get_phrase('hours') }}</button>
                            </div>
                        </div>
                    </div>
                </div>
...
@push('js')
    ...
    <script>
        ...
        function initiateTimesheetTotalHours() {
            var totalHours = 0;
            $('.total-hour-container').removeClass('d-none');

            const table = document.querySelector('.server-side-datatable');
            ...
            rows.forEach(row => {
                ...
                if (cells.length >= 6) {
                    ...
                    const numberMatch = cellText.match(/[\d.]+/); // Match float/double
                    if (numberMatch) {
                        totalHours += parseFloat(numberMatch[0]); // Convert to float
                    }
                }
            });

            $('#numberOfTotalHours').text(totalHours.toFixed(2)); // Round to 2 decimal places
        }
        ...
        initiate_floating_actions(
            ...
            "{{ has_permission('timesheet.delete') ? route(get_current_user_role() . '.timesheet.multi-delete') : '' }}", // Delete route
            "{{ route(get_current_user_role() . '.timesheet.export-file', ['file' => 'pdf', 'code' => request()->route()->parameter('code')]) }}", // Export route pdf
            "{{ route(get_current_user_role() . '.timesheet.export-file', ['file' => 'csv', 'code' => request()->route()->parameter('code')]) }}", // Export route csv
            "{{ route(get_current_user_role() . '.timesheet.export-file', ['file' => 'print', 'code' => request()->route()->parameter('code')]) }}", // Export route for print copy
        );

        server_side_datatable('["id","title","user","from","to","hours","options"]', "{{ route(get_current_user_role() . '.timesheets') }}", '{{ $total_timesheets }}');
    </script>
@endpush
