...
@endphp
@push('title', get_phrase('Invoice').' - '.$project->title)
                ...
                <div class="ol-card-body p-3 position-relative" id="filters-container">
                                ...
                                <div class="top-bar d-flex align-items-center w-100 flex-wrap">
                                    ...
                                    <div class="custom-dropdown" id="export-btn1">
                                        ...
                                        <ul class="dropdown-list dropdown-export">
                                            <li class="mb-1">
                                                <a class="dropdown-item export-btn" href="javascript:void(0)" onclick="exportFile('{{ route(get_current_user_role() . '.invoice.export-file', ['file' => 'pdf', 'code' => request()->route()->parameter('code')]) }}')">
                                                    <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                        <path
                                            ...
                                            <li class="mb-1">
                                                <a class="dropdown-item export-btn" href="javascript:void(0)" onclick="exportFile('{{ route(get_current_user_role() . '.invoice.export-file', ['file' => 'csv', 'code' => request()->route()->parameter('code')]) }}')">
                                            ...
                                            <li>
                                                <a class="dropdown-item export-btn" href="javascript:void(0)" onclick="exportFile('{{ route(get_current_user_role() . '.invoice.export-file', ['file' => 'print', 'code' => request()->route()->parameter('code')]) }}')">
                                                    <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    ...
                                    @if (has_permission('invoice.create'))
                                        <button onclick="rightCanvas('{{ route(get_current_user_role() . '.invoice.create', ['code' => request()->route()->parameter('code')]) }}', 'Create Invoice')" class="btn ol-btn-outline-secondary d-flex align-items-center cg-10px ms-auto enable-no-data-action">
                    ...
                    <div class="table-responsive">
                        <table class="table server-side-datatable" id="project_list">
                            ...
                            <tbody>
                                ...
                                @foreach ($query->limit(10)->get() as $key => $invoice)
                                    <tr>
                                        <td>
                                        ...
                                        <td>{{ date('d M Y h:i A', strtotime($invoice->timestamp_start)) }}</td>
                                        <td>
                                            @if ($invoice->due_date)
                                                {{ date('d M Y h:i A', strtotime($invoice?->due_date)) }}
                                            @endif
                                        </td>
                                        <td>
                                            @php
                                                ...
                                                switch ($invoice->payment_status) {
                                                    case 'processing':
                                                        $statusLabel = '<span class="processing">' . removeScripts(get_phrase('Processing')) . '</span>';
                                                        break;
                                                    case 'unpaid':
                                                        $statusLabel = '<span class="unpaid">' . removeScripts(get_phrase('Unpaid')) . '</span>';
                                                        break;
                                                    case 'paid':
                                                        $statusLabel = '<span class="completed">' . removeScripts(get_phrase('Completed')) . '</span>';
                                                        break;
                                                    default:
                                                        $statusLabel = '<span class="unknown">' . removeScripts(get_phrase('Unknown')) . '</span>';
                                                        break;
                                                }
                                            @endphp
                                        ...
                                        <td>
                                            @php
                                                $editRoute = route($current_user_role . '.invoice.edit', $invoice->id);
                                                $deleteRoute = route($current_user_role . '.invoice.delete', $invoice->id);
                                                $invoiceRoute = route($current_user_role . '.invoice.view', $invoice->id);
                                                $payoutRoute = route($current_user_role . '.invoice.payout', $invoice->id);
                                                $options = 0;
                                            @endphp
                                            ...
                                            <div class="dropdown disable-right-click ol-icon-dropdown">
                                                ...
                                                <ul class="dropdown-menu contextMenuContainer">
                                                    @if (has_permission('invoice.edit'))
                                                        @php $options++; @endphp
                                                        <li>
                                                            <a class="dropdown-item" onclick="rightCanvas('{{ $editRoute }}', '{{ get_phrase('Edit invoice') }}')" href="javascript:void(0)">{{ get_phrase('Edit') }}</a>
                                                        </li>
                                                    @endif
                                                    @if (has_permission('invoice.delete'))
                                                        @php $options++; @endphp
                                                        <li>
                                                            <a class="dropdown-item" onclick="confirmModal('{{ $deleteRoute }}')">{{ get_phrase('Delete') }}</a>
                                                        </li>
                                                    @endif
                                                    ...
                                                    @if (get_current_user_role() == 'client')
                                                        @php $options++; @endphp
                                                        <li>
                                                            <a class="dropdown-item" href="{{ $payoutRoute }}">{{ get_phrase('Checkout') }}</a>
                                                        </li>
                                                    @endif
                                                    @if ($options == 0)
                    ...
                    <div class="d-none d-lg-block custom-datatable-row-selector">
                        <div class="page-length-select fs-12px margin--40px d-flex align-items-center position-absolute">
                            ...
                            <label for="page-length-select" class="ps-2 w-100"> {{ get_phrase('of') . ' ' . $total_invoices }}</label>
                        </div>
                    </div>
...
@push('js')
    ...
    <script>
        ...
        initiate_floating_actions(
            ...
            "{{ has_permission('invoice.delete') ? route(get_current_user_role() . '.invoice.multi-delete'):'' }}", // Delete route
            "{{ route(get_current_user_role() . '.invoice.export-file', ['file' => 'pdf', 'code' => request()->route()->parameter('code')]) }}", // Export route pdf
            "{{ route(get_current_user_role() . '.invoice.export-file', ['file' => 'csv', 'code' => request()->route()->parameter('code')]) }}", // Export route pdf
            "{{ route(get_current_user_role() . '.invoice.export-file', ['file' => 'print', 'code' => request()->route()->parameter('code')]) }}", // Export route pdf
        );

        server_side_datatable('["id","title","payment","time","due_date","payment_status","options"]', "{{ route(get_current_user_role() . '.invoice') }}", '{{ $total_invoices }}');
    </script>
@endpush
