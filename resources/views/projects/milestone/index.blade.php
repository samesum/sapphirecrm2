...
@push('title', get_phrase('Milestone'). ' - '.$project->title)
            ...
            <div class="ol-card">
                <div class="ol-card-body p-3 position-relative" id="filters-container">
                                ...
                                <div class="top-bar d-flex align-items-center w-100 flex-wrap">
                                    ...
                                    <div class="custom-dropdown" id="export-btn1">
                                        <button class="dropdown-header btn ol-btn-light btn-squire" data-bs-toggle="tooltip" title="{{ get_phrase('Export') }}">
                                            <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <path
                                                    d="M16.994 7.86602C16.8049 7.39018 16.3515 7.08268 15.8398 7.08268H14.0957V3.3335C14.0957 2.64433 13.5348 2.0835 12.8457 2.0835H7.84566C7.15649 2.0835 6.59566 2.64433 6.59566 3.3335V7.0835H4.99308C4.48141 7.0835 4.02807 7.391 3.83891 7.86683C3.64974 8.34266 3.76814 8.87683 4.13981 9.22766L9.36808 14.1676C9.66225 14.4451 10.039 14.5835 10.4156 14.5835C10.7923 14.5835 11.169 14.4443 11.4632 14.1676L16.6907 9.22766C17.0648 8.87599 17.1824 8.34185 16.994 7.86602ZM16.1198 8.62097L10.8915 13.561C10.6265 13.8127 10.2081 13.8127 9.94222 13.561L4.71313 8.62097C4.52647 8.4443 4.5907 8.23348 4.61487 8.17348C4.6382 8.11265 4.73639 7.91602 4.99389 7.91602H7.01314C7.12397 7.91602 7.22981 7.87185 7.30814 7.79435C7.38648 7.71685 7.42981 7.61018 7.42981 7.49935V3.33268C7.42981 3.10268 7.61731 2.91602 7.84647 2.91602H12.8465C13.0756 2.91602 13.2631 3.10268 13.2631 3.33268V7.49935C13.2631 7.61018 13.3073 7.71602 13.3848 7.79435C13.4623 7.87269 13.569 7.91602 13.6798 7.91602H15.8407C16.0982 7.91602 16.1966 8.11348 16.2199 8.17348C16.2432 8.23432 16.3065 8.44514 16.1198 8.62097ZM16.6665 17.5002C16.6665 17.7302 16.4798 17.9168 16.2498 17.9168H4.58313C4.35313 17.9168 4.16646 17.7302 4.16646 17.5002C4.16646 17.2702 4.35313 17.0835 4.58313 17.0835H16.2498C16.4798 17.0835 16.6665 17.2702 16.6665 17.5002Z"
                                                    fill="#98A1B7" />
                                            </svg>
                                        </button>
                                        <ul class="dropdown-list dropdown-export">
                                            <li class="mb-1">
                                                <a class="dropdown-item export-btn" href="javascript:void(0)" onclick="exportFile('{{ route(get_current_user_role() . '.milestone.export-file', ['file' => 'pdf', 'code' => request()->route()->parameter('code')]) }}')">
                                                    <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                        <path
                                            ...
                                            <li class="mb-1">
                                                <a class="dropdown-item export-btn" href="javascript:void(0)" onclick="exportFile('{{ route(get_current_user_role() . '.milestone.export-file', ['file' => 'csv', 'code' => request()->route()->parameter('code')]) }}')">
                                            ...
                                            <li>
                                                <a class="dropdown-item export-btn" href="javascript:void(0)" onclick="exportFile('{{ route(get_current_user_role() . '.milestone.export-file', ['file' => 'print', 'code' => request()->route()->parameter('code')]) }}')">
                                                    <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    ...
                                    <div class="custom-dropdown dropdown filter-dropdown btn-group" id="export-btn">
                                        ...
                                        <div class="dropdown-list custom-dropdown-280px px-14px" aria-labelledby="filterDropdownButton" id="filter-section">
                                            <!-- project task -->
                                            <div class="mb-3">
                                                @php $tasks = App\Models\Task::where('project_id', project_id_by_code(request()->route()->parameter('code')))->get(); @endphp
                                                <label for="task" class="form-label">{{ get_phrase('Project Task') }}</label>
                                                <select class="form-control px-14px ol-form-control ol-select2" name="task" id="task">
                                                    <option value="all">{{ get_phrase('Select Task') }}</option>
                                            ...
                                            <div class="fpb7 mb-2">
                                                ...
                                                <br>
                                                <input class="form-control ol-form-control dateTimeRangePicker w-100" type="text" id="daterangepicker" value="" name="date_range" required>
                                            </div>
                                    ...
                                    @if (has_permission('milestone.create'))
                                        <button onclick="rightCanvas('{{ route(get_current_user_role() . '.milestone.create', ['code' => request()->route()->parameter('code')]) }}', 'Create milestone')" class="btn ol-btn-outline-secondary d-flex align-items-center cg-10px ms-auto enable-no-data-action">
                    ...
                    <div class="table-responsive">
                        <table class="table server-side-datatable milestone-table" id="project_list">
                            <thead>
                            ...
                            <tbody>
                                @php
                                    $milestones = App\Models\Milestone::where('project_id', project_id_by_code(request()->route()->parameter('code')));
                                    $total_milestones = $milestones->count();
                                @endphp
                                @foreach ($milestones->limit(10)->get() as $key => $milestone)
                                    <tr>
                                        <td>
                                        ...
                                        <td>
                                            <div class="ellipsis-line-1 mx-200px" data-bs-toggle="tooltip" title="{{ $milestone?->title }}">{{ $milestone?->title }}</div>
                                        </td>
                                        <td>
                                            @php $progress = $milestone->progress; @endphp
                                        ...
                                        <td>
                                            @php $tasks = $milestone?->tasks; @endphp

                                            @if (is_array($tasks) && count($tasks) > 0)
                                                <ul class="circle-style">
                                                    @foreach ($tasks as $task)
                                                        @php $task_title = App\Models\Task::where('id', $task)->first()?->title; @endphp
                                                        <li><span class="ellipsis-line-1 mx-400px" title="{{$task_title}}" data-bs-toggle="tooltip">{{ $task_title }}</span></li>
                                                    @endforeach
                                                </ul>
                                            @else
                                                {{ get_phrase('No tasks available') }}
                                            @endif
                                        </td>
                                        <td>
                                            <div class="ellipsis-line-1 mx-400px" data-bs-toggle="tooltip" title="{{$milestone?->description}}">
                                            ...
                                            <div class="dropdown disable-right-click ol-icon-dropdown">
                                                ...
                                                <ul class="dropdown-menu contextMenuContainer">
                                                    @if (has_permission('milestone.edit'))
                                                        @php ++$options; @endphp
                                                        <li>
                                                            <a class="dropdown-item" onclick="rightCanvas('{{ $editRoute }}', '{{ get_phrase('Edit milestone') }}')" href="javascript:void(0)">{{ get_phrase('Edit') }}</a>
                                                        </li>
                                                    @endif
                                                    ...
                                                    @endif
                                                    @if ($options == 0)
                                                        <li><a href="javascript:void(0)" class="dropdown-item">{{ get_phrase('No actions available') }}</span></li>
                                                    @endif
                                                </ul>
                                            </div>
                                        </td>
                    ...
                    <div class="d-none d-lg-block custom-datatable-row-selector">
                        <div class="page-length-select fs-12px margin--40px d-flex align-items-center position-absolute">
                            ...
                            <label for="page-length-select" class="ps-2 w-100"> {{ get_phrase('of') }} {{ $total_milestones }}</label>
                        </div>
                    </div>
...
@push('js')
    ...
    <script>
        ...
        initiate_floating_actions(
            ...
            "{{ has_permission('milestone.delete') ? route(get_current_user_role() . '.milestone.multi-delete'):'' }}", // Delete route
            "{{ route(get_current_user_role() . '.milestone.export-file', ['file' => 'pdf', 'code' => request()->route()->parameter('code')]) }}", // Export route pdf
            "{{ route(get_current_user_role() . '.milestone.export-file', ['file' => 'csv', 'code' => request()->route()->parameter('code')]) }}", // Export route csv
            "{{ route(get_current_user_role() . '.milestone.export-file', ['file' => 'print', 'code' => request()->route()->parameter('code')]) }}", // Export route for print copy
        );

        server_side_datatable('["id","title","progress","tasks","description","options"]', "{{ route(get_current_user_role() . '.milestones') }}", "{{ $total_milestones }}");
    </script>
@endpush
