...
@push('title', get_phrase('Manage Addons'))
@section('content')
                    ...
                    <div class="ol-card-body p-3 position-relative" id="filters-container">
                                    ...
                                    <div class="top-bar d-flex align-items-center w-100 flex-wrap">
                                        ...
                                        <div class="custom-dropdown" id="export-btn1">
                                            ...
                                            <ul class="dropdown-list dropdown-export">
                                                <li class="mb-1">
                                                    <a class="dropdown-item export-btn" onclick="exportFile('{{ route(get_current_user_role() . '.addon.export-file', ['file' => 'pdf']) }}')" href="javascript:void(0)">
                                                ...
                                                <li class="mb-1">
                                                    <a class="dropdown-item export-btn" href="javascript:void(0)" onclick="exportFile('{{ route(get_current_user_role() . '.addon.export-file', ['file' => 'csv']) }}')">
                                                ...
                                                <li>
                                                    <a class="dropdown-item export-btn" target="_blank" href="javascript:void(0)" onclick="exportFile('{{ route(get_current_user_role() . '.addon.export-file', ['file' => 'print']) }}')"><i class="fi-rr-print"></i>
                                                        {{ get_phrase('Print') }}</a>
                                                </li>
                                            </ul>
                                        ...
                                        @if (has_permission('addon.add'))
                                            <button onclick="rightCanvas('{{ route(get_current_user_role() . '.addon.add') }}', 'Addon Upload')" class="btn ol-btn-outline-secondary d-flex align-items-center cg-10px enable-no-data-action ms-auto">
                        ...
                        <table class="table server-side-datatable" id="project_list">
                            ...
                            <tbody>
                                ...
                                @endphp
                                @foreach (App\Models\Addon::limit(10)->get() as $key => $addon)
                                    <tr>
                                        <td>
                                        ...
                                        <td>
                                            ...
                                            @else
                                                <span class="not_started">{{ get_phrase('Deactive') }}</span>
                                            @endif
                                        </td>
                                        <td>
                                            @php
                                                $statusRoute = route($current_user_role . '.addon.status', $addon->id);
                                                $editRoute = route($current_user_role . '.addon.edit', $addon->id);
                                                $deleteRoute = route($current_user_role . '.addon.delete', $addon->id);
                                                $options = 0;
                                            @endphp
                                            ...
                                            <div class="dropdown disable-right-click ol-icon-dropdown">
                                                ...
                                                <ul class="dropdown-menu contextMenuContainer">
                                                    @if (has_permission('addon.status'))
                                                        @php ++$options; @endphp
                                                        <li>
                                                            <a class="dropdown-item" onclick="confirmModal('{{ $statusRoute }}')" href="javascript:void(0)">{{ $addon->status ? get_phrase('Disable addon') : get_phrase('Activate addon') }}</a>
                                                        </li>
                                                    @endif
                                                    @if (has_permission('addon.edit'))
                                                        @php ++$options; @endphp
                                                        <li>
                                                            <a class="dropdown-item" onclick="rightCanvas('{{ $editRoute }}', '{{ get_phrase('Edit addon') }}')" href="javascript:void(0)">{{ get_phrase('Edit') }}</a>
                                                        </li>
                                                    @endif
...
@push('js')
    <script>
        ...
        initiate_floating_actions(
            ...
            "", // Delete route
            "{{ route(get_current_user_role() . '.addon.export-file', ['file' => 'pdf']) }}", // Export route pdf
            "{{ route(get_current_user_role() . '.addon.export-file', ['file' => 'csv']) }}", // Export route csv
            "{{ route(get_current_user_role() . '.addon.export-file', ['file' => 'print']) }}", // Export route for print copy
        );

        server_side_datatable('["id","name","version","status","options"]', "{{ route(get_current_user_role() . '.addons') }}", '{{ App\Models\Addon::count() }}');
